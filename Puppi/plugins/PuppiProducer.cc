...
/*	edm::Handle<reco::VertexCollection> hVertexProduct;
	...
	for(CandidateView::const_iterator itPF = PFCol->begin(); itPF!=PFCol->end(); itPF++) {
		...
		const edm::Ptr<pat::PackedCandidate > lPackEdmPtr = (edm::Ptr<pat::PackedCandidate >&) lPack; //This is needed otherwise using std::count below gives 500+ lines of compilation error.
		...
		assert(diPhotonPointers.size() >0); //avoid segfaults
		const edm::Ptr<reco::Vertex> lpv = diPhotonPointers[0]->getVertex(); // Teh idea is to eventually loop over ALL diphoton canidates in future commit. FIXME
	...
	//fPuppiCandidates.reset( new vector<edm::FwdPtr<reco::PFCandidate> > );    
